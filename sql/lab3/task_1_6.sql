...
WHERE f.departure_airport = 'VVO'
    ...
    AND EXTRACT(EPOCH FROM (CURRENT_DATE - COALESCE(actual_departure, scheduled_departure))) = (
        SELECT
            MIN(EXTRACT(EPOCH FROM (CURRENT_DATE - COALESCE(actual_departure, scheduled_departure))))
        FROM flights f_t
